...
const CreditCards = () => {
  ...
  const {
    ...
    pageChange,
    rowChange
  } = useFistoHook("/api/admin/credit-card")
  ...
  const dataStatusHandler = (data) => {
    ...
    setConfirm({
      ...
      onConfirm: async () => {
        ...
        let response
        try {
          response = await axios.patch(`/api/admin/credit-card/${id}`, {
  ...
  const dataUpdateHandler = (data) => {
    const { id, name, account_no, utility_categories, utility_locations } = data
    setUpdate({
      id,
      name,
      account_no,
