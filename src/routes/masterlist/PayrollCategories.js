...
const PayrollCategories = () => {
  ...
  // Payroll Categories Array
  const [payrollCategories, setPayrollCategories] = React.useState(null)
  // Pagination Object
  ...
  React.useEffect(() => {
    ...
    if (mode) fetchPayrollCategories(true)
    else  fetchPayrollCategories(false)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  const fetchPayrollCategories = async (status) => {
    setIsFetching(true)

    let response
    try {
      response = await axios.get(`/api/payroll-category/${status ? 1 : 0}/${pagination ? pagination.per_page : 10}/`).then(json => json.data)
      const { data, ...paginate } = response.result

      setPayrollCategories(data)
      ...
      if (error.request.status !== 404) {
        setToast({
          show: true,
          title: "Error",
          message: "Something went wrong whilst fetching payroll categories.",
          severity: "error"
        })
      }

      setPayrollCategories(null)
      setPagination(null)
  ...
  const modeChangeHandler = () => {
    ...
    if (mode) {
      setMode(false)
      fetchPayrollCategories(false)
    }
    else {
      setMode(true)
      fetchPayrollCategories(true)
    }
  }
  ...
  const searchSubmitHandler = async (e) => {
    if (e.key === "Enter")
    {
      ...
      try {
        if (mode) {
          response = await axios.post(`/api/payroll-category/search/1/${pagination ? pagination.per_page : 10}/`, {
            value: isSearching.keyword
          })
          .then(res => res.data)
        }
        else {
          response = await axios.post(`/api/payroll-category/search/0/${pagination ? pagination.per_page : 10}/`, {
            value: isSearching.keyword
          })
          .then(res => res.data)
        }
      ...
      onConfirm: async () => {
        ...
        try {
          if (isUpdating.status) {
            response = await axios.put(`/api/payroll-category/${isUpdating.data}/`, {
              category: payrollCategory.category
            })
            .then(res => res.data)
          }
          else {
            response = await axios.post(`/api/payroll-category/`, {
              category: payrollCategory.category
            })
            .then(res => res.data)
          }
          ...
          if (status === 409) {
            ...
            setToast({
              show: true,
              title: "Error",
              message: "Something went wrong whilst saving payrol category.",
              severity: "error"
            })
        }
  ...
  const pageChangeHandler = async (e, page) => {
    ...
    try {
      if (mode && isSearching.status) {
        response = await axios.post(`/api/payroll-category/search/1/${pagination ? pagination.per_page : 10}?page=${++page}`, {
          value: isSearching.keyword
        })
        .then(res => res.data)
      }
      else if (!mode && isSearching.status) {
        response = await axios.post(`/api/payroll-category/search/0/${pagination ? pagination.per_page : 10}?page=${++page}`, {
          value: isSearching.keyword
        })
        .then(res => res.data)
      }
      else if (mode) {
        response = await axios.get(`api/payroll-category/1/${pagination ? pagination.per_page : 10}?page=${++page}`).then(res => res.data)
      }
      else if (!mode) {
        response = await axios.get(`api/payroll-category/0/${pagination ? pagination.per_page : 10}?page=${++page}`).then(res => res.data)
      }
  ...
  const rowChangeHandler = async (e) => {
    ...
    try {
      if (mode && isSearching.status) {
        response = await axios.post(`/api/payroll-category/search/1/${e.target.value}/`, {
          value: isSearching.keyword
        })
        .then(res => res.data)
      }
      else if (!mode && isSearching.status) {
        response = await axios.post(`/api/payroll-category/search/0/${e.target.value}/`, {
          value: isSearching.keyword
        })
        .then(res => res.data)
      }
      else if (mode) {
        response = await axios.get(`/api/payroll-category/1/${e.target.value}/`).then(res => res.data)
      }
      else if (!mode)  {
        response = await axios.get(`/api/payroll-category/0/${e.target.value}/`).then(res => res.data)
      }
  ...
  const actionArchiveHandler = ({ id }) => {
    setConfirm({
      ...
      onConfirm: async () => {
        ...
        let response
        try {
          response = await axios.post(`/api/payroll-category/archive/${id}/`).then(res => res.data)
  ...
  const actionRestoreHandler = ({ id }) => {
    setConfirm({
      ...
      onConfirm: async () => {
        ...
        let response
        try {
          response = await axios.post(`/api/payroll-category/restore/${id}/`).then(res => res.data)
      ...
      <Paper className="FstoPaperTable-root" elevation={1}>
        <Box className="FstoBoxToolbar-root">
          <Box className="FstoBoxToolbar-left">
            <Typography variant="heading">Payroll Category</Typography>
          </Box>
            ...
            <TableBody>
              {
                ...
                : payrollCategories
                  ? payrollCategories.map((data, index) => <TableData key={index} data={data} />)
                  : (
                      <TableRow>
