...
import SubUnitsForm from './SubUnitsForm'
import SubUnitsTable from './SubUnitsTable'
import SubUnitsToolbar from './SubUnitsToolbar'
...
const SubUnits = () => {
  ...
  const {
    ...
    pageChange,
    rowChange
  } = useFistoHook("/api/admin/sub-units")
  ...
  const dataStatusHandler = (data) => {
    ...
    setConfirm({
      ...
      onConfirm: async () => {
        ...
        let response
        try {
          response = await axios.patch(`/api/admin/sub-units/${id}`, {
