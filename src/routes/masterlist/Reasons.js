...
const Reasons = () => {
  ...
  React.useEffect(() => {
    ...
    if (mode) fetchReasons(true)
    else  fetchReasons(false)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  const fetchReasons = async (status) => {
    setIsFetching(true)

    let response
    try {
      response = await axios.get(`/api/reasons/${status ? 1 : 0}/${pagination ? pagination.per_page : 10}/`).then(json => json.data)
      ...
      if (error.request.status !== 404) {
        setToast({
          show: true,
          title: "Error",
          message: "Something went wrong whilst fetching reasons.",
          severity: "error"
        })
      }
  ...
  const searchSubmitHandler = async (e) => {
    if (e.key === "Enter")
    {
      ...
      try {
        if (mode) {
          response = await axios.post(`/api/reasons/search/1/${pagination ? pagination.per_page : 10}/`, {
            value: isSearching.keyword
          })
          .then(res => res.data)
        }
        else {
          response = await axios.post(`/api/reasons/search/0/${pagination ? pagination.per_page : 10}/`, {
            value: isSearching.keyword
          })
          .then(res => res.data)
        }
      ...
      onConfirm: async () => {
        ...
        try {
          if (isUpdating.status) {
            response = await axios.put(`/api/reasons/${isUpdating.data}/`, {
            ...
            response = await axios.post(`/api/reasons/`, {
              reason: reason.reason,
              remarks: reason.remarks
            })
            .then(res => res.data)
          ...
          if (status === 409) {
            ...
            setToast({
              show: true,
              title: "Error",
              message: "Something went wrong whilst saving reason.",
              severity: "error"
            })
        }
  ...
  const pageChangeHandler = async (e, page) => {
    ...
    try {
      if (mode && isSearching.status) {
        response = await axios.post(`/api/reasons/search/1/${pagination ? pagination.per_page : 10}?page=${++page}`, {
          value: isSearching.keyword
        })
        .then(res => res.data)
      }
      else if (!mode && isSearching.status) {
        response = await axios.post(`/api/reasons/search/0/${pagination ? pagination.per_page : 10}?page=${++page}`, {
          value: isSearching.keyword
        })
        .then(res => res.data)
      }
      else if (mode) {
        response = await axios.get(`api/reasons/1/${pagination ? pagination.per_page : 10}?page=${++page}`).then(res => res.data)
      }
      else if (!mode) {
        response = await axios.get(`api/reasons/0/${pagination ? pagination.per_page : 10}?page=${++page}`).then(res => res.data)
      }
  ...
  const rowChangeHandler = async (e) => {
    ...
    try {
      if (mode && isSearching.status) {
        response = await axios.post(`/api/reasons/search/1/${e.target.value}/`, {
          value: isSearching.keyword
        })
        .then(res => res.data)
      }
      else if (!mode && isSearching.status) {
        response = await axios.post(`/api/reasons/search/0/${e.target.value}/`, {
          value: isSearching.keyword
        })
        .then(res => res.data)
      }
      else if (mode) {
        response = await axios.get(`/api/reasons/1/${e.target.value}/`).then(res => res.data)
      }
      else if (!mode)  {
        response = await axios.get(`/api/reasons/0/${e.target.value}/`).then(res => res.data)
      }
  ...
  const actionArchiveHandler = ({ id }) => {
    setConfirm({
      ...
      onConfirm: async () => {
        ...
        let response
        try {
          response = await axios.post(`/api/reasons/archive/${id}/`).then(res => res.data)
  ...
  const actionRestoreHandler = ({ id }) => {
    setConfirm({
      ...
      onConfirm: async () => {
        ...
        let response
        try {
          response = await axios.post(`/api/reasons/restore/${id}/`).then(res => res.data)
      ...
      <Paper className="FstoPaperTable-root" elevation={1}>
        <Box className="FstoBoxToolbar-root">
          <Box className="FstoBoxToolbar-left">
            <Typography variant="heading">Reasons</Typography>
          </Box>
            ...
            <TableHead>
              <TableRow>
                ...
                <TableCell className="FstoTableHead-root">
                  <TableSortLabel active={true}>REASON</TableSortLabel>
                </TableCell>

                <TableCell className="FstoTableHead-root">
                  <TableSortLabel active={true}>REMARKS</TableSortLabel>
                </TableCell>
            ...
            <TableBody>
              {
                ...
                : reasons
                  ? reasons.map((data, index) => <TableData key={index} data={data} />)
                  : (
                      <TableRow>
