...
const SupplierTypes = () => {
  ...
  // Supplier Types Array
  const [supplierTypes, setSupplierTypes] = React.useState(null)
  // Pagination Object
  ...
  React.useEffect(() => {
    ...
    if (mode) fetchSupplierTypes(true)
    else  fetchSupplierTypes(false)

    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  const fetchSupplierTypes = async (status) => {
    setIsFetching(true)

    let response
    try {
      response = await axios.get(`api/supplier-types/${status ? 1 : 0}/${pagination ? pagination.per_page : 10}/`).then(res => res.data)
      ...
      if (error.request.status !== 404) {
        setToast({
          show: true,
          title: "Error",
          message: "Something went wrong whilst fetching supplier types.",
          severity: "error"
        })
      }
  ...
  const modeChangeHandler = () => {
    ...
    if (mode) {
      setMode(false)
      fetchSupplierTypes(false)
    }
    else {
      setMode(true)
      fetchSupplierTypes(true)
    }
  }
  ...
  const searchSubmitHandler = async (e) => {
    if (e.key === "Enter")
    {
      ...
      try {
        if (mode) {
          response = await axios.post(`/api/supplier-types/search/1/${pagination ? pagination.per_page : 10}/`, {
            value: isSearching.keyword
          })
          .then(res => res.data)
        }
        else {
          response = await axios.post(`/api/supplier-types/search/0/${pagination ? pagination.per_page : 10}/`, {
            value: isSearching.keyword
          })
          .then(res => res.data)
        }
      ...
      onConfirm: async () => {
        ...
        try {
          if (isUpdating.status) {
            response = await axios.put(`/api/supplier-types/${isUpdating.data}/`, {
            ...
            response = await axios.post(`/api/supplier-types/`, {
              type: supplierType.type,
              transaction_days: supplierType.transaction
            })
            .then(res => res.data)
  ...
  const pageChangeHandler = async (e, page) => {
    ...
    try {
      if (mode && isSearching.status) {
        response = await axios.post(`/api/supplier-types/search/1/${pagination ? pagination.per_page : 10}?page=${++page}`, {
          value: isSearching.keyword
        })
        .then(res => res.data)
      }
      else if (!mode && isSearching.status) {
        response = await axios.post(`/api/supplier-types/search/0/${pagination ? pagination.per_page : 10}?page=${++page}`, {
          value: isSearching.keyword
        })
        .then(res => res.data)
      }
      else if (mode) {
        response = await axios.get(`api/supplier-types/1/${pagination ? pagination.per_page : 10}?page=${++page}`).then(res => res.data)
      }
      else if (!mode) {
        response = await axios.get(`api/supplier-types/0/${pagination ? pagination.per_page : 10}?page=${++page}`).then(res => res.data)
      }
  ...
  const rowChangeHandler = async (e) => {
    ...
    try {
      if (mode && isSearching.status) {
        response = await axios.post(`/api/supplier-types/search/1/${e.target.value}/`, {
          value: isSearching.keyword
        })
        .then(res => res.data)
      }
      else if (!mode && isSearching.status) {
        response = await axios.post(`/api/supplier-types/search/0/${e.target.value}/`, {
          value: isSearching.keyword
        })
        .then(res => res.data)
      }
      else if (mode) {
        response = await axios.get(`/api/supplier-types/1/${e.target.value}/`).then(res => res.data)
      }
      else if (!mode)  {
        response = await axios.get(`/api/supplier-types/0/${e.target.value}/`).then(res => res.data)
      }
  ...
  const actionArchiveHandler = ({ id }) => {
    setConfirm({
      ...
      onConfirm: async () => {
        ...
        let response
        try {
          response = await axios.post(`/api/supplier-types/archive/${id}/`).then(res => res.data)
  ...
  const actionRestoreHandler = ({ id }) => {
    setConfirm({
      ...
      onConfirm: async () => {
        ...
        let response
        try {
          response = await axios.post(`/api/supplier-types/restore/${id}/`).then(res => res.data)
  ...
  const actionUpdateHandler = (data) => {
    const { id, type, transaction_days: transaction } = data
            ...
            <TableHead>
              <TableRow>
                ...
                <TableCell className="FstoTableHead-root">
                  <TableSortLabel active={true}>SUPPLIER TYPE</TableSortLabel>
                </TableCell>

                <TableCell className="FstoTableHead-root">
                  <TableSortLabel active={true}>TRANSACTION DAYS</TableSortLabel>
                </TableCell>
            ...
            <TableBody>
              {
                ...
                : (
                    supplierTypes
                    ? supplierTypes.map((data, index) => <TableData key={index} data={data} />)
                    : (
                        <TableRow>
