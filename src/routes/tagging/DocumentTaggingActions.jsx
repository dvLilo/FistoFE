...
const DocumentTaggingActions = ({ data, state, onReceive, onManage, onView, onCancel }) => {
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        {
          (state === `tag-tag` || state === `tag-hold` || state === `tag-return` || state === `tag-void`) &&
          <MenuItem
            sx={{ fontWeight: 500 }}
            onClick={() => {
              onView(data)
...
export default DocumentTaggingActions