...
const DocumentReversing = () => {
  ...
  const {
    ...
    changePage,
    changeRows
  } = useTransactions("/api/transactions", Boolean(user?.role.match(/ap associate/i)) ? "pending-request" : "pending-file")
  ...
  const [state, setState] = React.useState(Boolean(user?.role.match(/ap associate/i)) ? "pending-request" : "pending-file")
